...
typedef struct LoopList {
    ...
    char loopType[60]; //For /While //Switch
    char desc[60];
...
void addToLoopList(LoopList *list,char loopType[60],bool isMethod,char desc[60])
{
    if(list->first == true)//ilk eleman
    {
        memset(list->loopType,0,60);
        memset(list->desc,0,60);
    ...
    else
    {
        ...
        if(list->next == NULL)
        {
            ...
            memset(list->next->loopType,0,60);
            memset(list->next->desc,0,60);
...
{
    int maxTimeComplexivity = 0;
    while(list != NULL)
    {
        if(list->timeComplexity > maxTimeComplexivity)
            maxTimeComplexivity = list->timeComplexity;
        list = list->next;
    }


        maxTimeComplexivity -= recursive;


    return (maxTimeComplexivity);
}


int metodCheck(const char processedInput[60],char desc[60],int start,LoopList *list)
{
    char metodType[60];
    memset(metodType, 0, 60);
...
void findDescription(char processedInput[60],LoopList *list,int i)
{
    char desc[60];
    memset(desc, 0, 60);
    int start,finish;
...
void findEndBrackets(char processedInput[60],int i,LoopList *list) //Metodun nerede bittigini bulur
{
    ...
    for (int j = i; j < strlen(processedInput); j++) {
        ...
        if('}' == processedInput[j])
        {
            ...
            //algoritmik karmaşıklık hesaplayici !!
              ...
              if(lastOpenLoop->parent != NULL)
              {
                  if(lastOpenLoop->parent->timeComplexity - lastOpenLoop->timeComplexity < 1)
                  lastOpenLoop->parent->timeComplexity = lastOpenLoop->parent->timeComplexity + lastOpenLoop->timeComplexity;

              }

            if(strcmp(lastOpenLoop->loopType,"switch") == 0 || strcmp(lastOpenLoop->loopType,"if") == 0)
            {
                list = lastOpenLoop->previous;
                free(lastOpenLoop->next);
                lastOpenLoop->next = NULL;
...
void findStartBrackets(char processedInput[60],int i,LoopList *list) //Metodun nerede basladigini bulur
{
    if(list->loopID < 1)
        return;
    for (int j = i; j < strlen(processedInput); j++) {
...
void definitionChecker(char processedInput[60],LoopList *list,int start,int veriBoyutu) //Return 1 metod baslangici
{
...
void findComplexityChange(char processedInput[60],LoopList *list)
{
...
void findSwitch(char processedInput[60],LoopList *list)
{
...
void findIf(char processedInput[60],LoopList *list)
{
...
void findWhile(char processedInput[60],LoopList *list)
{
...
void findFor(char processedInput[60],LoopList *list)
{
...
void findDo(char processedInput[60],LoopList *list)
{
...
void findComponents(char processedInput[60],LoopList *list)
{
...
void getLine()
{
    ...
    while (!feof(input))
    {
        memset(rawInput, 0, 60);
        memset(processedInput, 0, 60); //Listeyi tamamen sifirlar

        fgets(rawInput,60,input);
